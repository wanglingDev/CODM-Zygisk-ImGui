#pragma once
// NOTE: Vector3, Vector2, Quaternion are defined in Include/Vector3.h,
// Include/Unity.h, Include/Quaternion.h (members X,Y,Z,W – uppercase).
...
#include <cinttypes>  // PRIxPTR — portable uintptr_t format specifier
...
struct Il2CppString {
    ...
    void* monitor;
    int   length;
    // FIXED: was wchar_t (4 bytes on Android = UTF-32).
    // IL2CPP string data is UTF-16 (uint16_t = 2 bytes).
    // wchar_t caused every character to consume 4 bytes instead of 2,
    // reading 2 code units at a time — all non-trivial names were garbage.
    uint16_t chars[1];
    std::string toString() {
        std::string s;
        s.reserve(length);
...
// Slerp helper for smooth aimbot
...
void DoAimbot(uintptr_t target) {
    ...
    // FIXED: fAimSmooth slider was never applied — aimbot snapped instantly.
    // Now lerp: smooth=1 → instant snap, smooth=10 → slow track.
    // t = 1/smooth so higher smooth value = slower rotation.
    float t = 1.0f / fAimSmooth;
    if (t >= 1.0f) {
        SetPawnAimRotation(local, desired);
    } else {
        // Need current rotation — read it back from pawn aim state.
        // Approximate: slerp toward desired with t factor each frame.
        // This gives natural easing without needing a GetAimRotation RVA.
