...
//  EGL HOOK STATE
// ════════════════════════════════════════════════════════════════
// orig_eglSwapBuffers — defined as non-static global in hook.cpp, extern'd here.
extern EGLBoolean (*orig_eglSwapBuffers)(EGLDisplay, EGLSurface);
// Backward-compat alias for older code that used old_eglSwapBuffers name:
...
EGLBoolean hook_eglSwapBuffers(EGLDisplay display, EGLSurface surface) {
    ...
    if (!g_imgui_init) {
        ...
        IMGUI_CHECKVERSION();
        ImGui::CreateContext();
        SetupImGuiTouchConfig();  // TouchExtraPadding + NoMouseCursorChange
        ImGuiIO& io = ImGui::GetIO();
